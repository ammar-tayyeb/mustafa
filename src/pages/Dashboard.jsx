...
//       setLoading(true);
//       setError(null);
//       const data = await getDashboardStats({ from: from || null, to: to || null });
//       setStats(data);
//     } catch (e) {
//       setError(e.message);
//     } finally {
//       setLoading(false);
//     }
//   }, [from, to]); // لن تتغير هذه الدالة إلا إذا تغير التاريخ

//   useEffect(() => {
//     load();
//   }, [load]); // الآن الـ effect يعتمد على دالة ثابتة، ولن يتكرر إلا عند تغير التاريخ

//   if (loading) return <div className="text-center py-20 text-muted-foreground">جارٍ التحميل...</div>;
//   if (error) return <div className="rounded-md bg-destructive/10 text-destructive px-4 py-3 text-sm">خطأ: {error}</div>;
//   if (!stats) return null;

//   return (
//     <div className="flex flex-col gap-6">
//       {/* رأس الصفحة وفلتر الفترة */}
//       <div className="flex items-center justify-between flex-wrap gap-3">
//         <div>
//           <h2 className="text-xl font-bold">لوحة المعلومات</h2>
//           <p className="text-sm text-muted-foreground mt-0.5">ملخص مالي وتشغيلي</p>
...
//         <StatCard icon={Package} label="إجمالي الحمالية" value={formatMoney(stats.totalPort)} color="text-amber-500" />
//         <StatCard icon={AlertCircle} label="إجمالي الديون" value={formatMoney(stats.totalDebt)} color="text-destructive" />
//         <StatCard icon={FileText} label="الفواتير" value={`${stats.postedCount} مُرحّلة`} sub={`${stats.draftCount} مسودة`} color="text-green-600" />
//         <StatCard icon={Users} label="التجار" value={stats.tradersCount} color="text-blue-500" />
//       </div>
