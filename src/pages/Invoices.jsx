...
import { Plus, Trash2, RotateCcw, CheckCircle, Printer, PlusCircle, Check } from "lucide-react";
import {
  ...
  getTraders, getActiveDrivers, openDriverSheet, getAllSettings,
} from "../lib/db.js";
...
// ─── صف البند مع السائق من SmallProductCombobox ──────────────────────
function ItemRow({
  item, productItems, activeDriverItems, defaultCommission, defaultBasketWeightEach,
  defaultBasketPrice, defaultPorterage, onChange, onRemove
}) {
  ...
  function field(key, rawVal) {
    ...
    if (["grossWeight","price","basketPrice","manualFinal","basketCount","porterage","commissionRate","basketWeightEach"].includes(key)) {
    ...
    const currentPortVal       = updated.porterage !== "" ? cleanCommas(updated.porterage) : defaultPorterage;
    ...
    const computed = computeInvoiceItem({
      ...
      basketPrice: cleanBasketPrice || "", commissionRate: cleanCommRate,
      porterage: totalPorterage, manualFinal: cleanManualFinal,
    });
    ...
    <div className="rounded border border-border bg-muted/20 p-1 flex flex-col gap-0.5 text-[11px]">
      ...
      <div className="grid grid-cols-5 gap-1">
        ...
        <div>
          ...
          <input type="text" value={item.grossWeight} onChange={e => field("grossWeight", e.target.value)} className={textInputClass} placeholder="0" />
        </div>
        <div>
          <span className={labelClass}>السعر</span>
          <input type="text" value={item.price} onChange={e => field("price", e.target.value)} className={textInputClass} placeholder="0" />
        </div>
        <div>
          <span className={labelClass}>عدد السلات</span>
          <input type="text" value={item.basketCount} onChange={e => field("basketCount", e.target.value)} className={textInputClass} placeholder="0" />
        </div>
      ...
      <div className="grid grid-cols-5 gap-1 items-end">
        <div>
          ...
          <input type="text" value={item.commissionRate} onChange={e => field("commissionRate", e.target.value)} className={textInputClass} placeholder={`${defaultCommission / 100}`} />
        </div>
        <div>
          <span className={labelClass}>وزن السلة</span>
          <input type="text" value={item.basketWeightEach} onChange={e => field("basketWeightEach", e.target.value)} className={textInputClass} placeholder="0" />
        </div>
        <div>
          <span className={labelClass}>حمالية / سلة</span>
          <input type="text" value={item.porterage} onChange={e => field("porterage", e.target.value)} className={textInputClass} placeholder="0" />
        </div>
        <div>
          <span className={labelClass}>سعر السلة</span>
          <input type="text" value={item.basketPrice} onChange={e => field("basketPrice", e.target.value)} className={textInputClass} placeholder="0" />
        </div>
        <div>
          <span className={labelClass}>نهائي (يدوي)</span>
          <input type="text" value={item.manualFinal} onChange={e => field("manualFinal", e.target.value)} className={`${textInputClass} border-dashed`} placeholder="تلقائي" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-1 items-end mt-0.5">
        <div className="col-span-2">
          <span className={`${labelClass} text-right`}>السائق (قائمة مفتوحة)</span>
          <SmallProductCombobox
            items={activeDriverItems}
            value={item.driver_id || ""}
            onChange={(id, label) => onChange({ ...item, driver_id: id, driver_label: label })}
      ...
      {c && (
        <div className="grid grid-cols-6 gap-0.5 rounded bg-muted/60 px-1 py-0.5 text-[9.5px] font-sans font-medium mt-0.5">
          <div className="text-center"><span className="text-muted-foreground font-normal">صافي: </span><span>{formatMoney(c.display.netWeight, "")}</span></div>
          <div className="text-center"><span className="text-muted-foreground font-normal">قبل: </span><span>{formatMoney(c.display.amountBefore, "")}</span></div>
          <div className="text-center"><span className="text-muted-foreground font-normal">عمولة: </span><span className="text-orange-600">{formatMoney(c.display.commissionValue, "")}</span></div>
          <div className="text-center"><span className="text-muted-foreground font-normal">سلة: </span><span className="text-violet-600">{formatMoney(c.display.basketPriceTotal, "")}</span></div>
          <div className="text-center"><span className="text-muted-foreground font-normal">حمالية: </span><span className="text-blue-600">{currentTotalPorterage.toLocaleString("en-US", { numberingSystem: "latn" })}</span></div>
          <div className="text-center"><span className="text-muted-foreground font-normal">نهائي: </span><span className="font-bold text-primary">{formatMoney(c.display.finalAmount, "")}</span></div>
        </div>
      )}
    </div>
  );
...
export default function Invoices() {
  ...
  const [invForm, setInvForm]     = useState({ trader_id: null, trader_label: "", notes: "" });
  ...
  const load = useCallback(async () => {
    try {
      ...
      setInvoices(inv); setTraders(tr); setActiveDrivers(dr); setSettings(st);
  ...
  const handlePayAll = () => {
    if (liveTotals.total_final > 0) setPaidAmount(Math.round(liveTotals.total_final).toString());
  };
  ...
  async function openEdit(inv) {
    ...
    setEditInv(inv);
    setInvForm({ trader_id: inv.trader_id, trader_label: inv.trader_name ?? "", notes: inv.notes ?? "" });
    setItems(its.map(it => ({
      _key: it.id,
      id: it.id,
  ...
  async function handlePrintDirectly(inv) {
    const its = await getInvoiceItems(inv.id);
    setViewInv(inv); setViewItems(its);
  }
  ...
  if (viewInv && viewItems.length > 0) {
    ...
    // تأجيل تصفير الحالة للدورة القادمة لتجنب تصادم الـ Renders
    setTimeout(() => {
      setViewInv(null);
      setViewItems([]);
    }, 0);
  ...
  async function handleSave(e) {
    e.preventDefault();
    if (!invForm.trader_label?.trim()) { alert("اسم البگال مطلوب"); return; }
    setSaving(true);
    try {
      ...
      if (!invoiceId) {
        invoiceId = await createInvoice({
          trader_id, driver_id: null, date: getLocalDateTimeString(), notes: invForm.notes || null
        });
      }

      // ─── معالجة البنود مع السواق ──────────────────────────────────
      const processedItems = [];
      for (const item of items) {
        if (!item.product_name.trim()) continue;
        let driverId = item.driver_id;
        // إذا كُتب اسم سائق جديد بدون اختيار من القائمة
        if (!driverId && item.driver_label?.trim()) {
          driverId = await findOrCreateDriver(item.driver_label.trim());
        }
      ...
      for (const item of processedItems) {
        ...
        const currentPortVal       = item.porterage !== "" ? cleanCommas(item.porterage) : defaultPorterage;
        const totalPorterage       = currentPortVal * cleanBasketCount;
        ...
        const id = await upsertInvoiceItem({
          ...computed, id: item.id, invoice_id: invoiceId,
          product_name: item.product_name, driver_id: item.driver_id ?? null,
        });
  ...
  function updateItem(key, updated) { setItems(prev => prev.map(it => it._key === key ? updated : it)); }
  function removeItem(key) { setItems(prev => prev.filter(it => it._key !== key)); }

  const formatViewDateTime = (dateStr) => {
    if (!dateStr) return "—";
    const d = new Date(dateStr);
    if (isNaN(d.getTime())) return dateStr;
    return `${d.toLocaleTimeString("ar-IQ", { hour: "2-digit", minute: "2-digit", hour12: false })} | ${d.toLocaleDateString("ar-IQ")}`;
  };
  ...
  const renderTableDateTime = (dateStr) => {
    ...
    return (
      <div className="flex flex-col text-center font-sans font-medium text-[10px] leading-tight select-none px-0.5">
        <span className="text-foreground/90">{d.toLocaleDateString("en-US", { year: "numeric", month: "2-digit", day: "2-digit" }).replace(/\//g, "-")}</span>
        <span className="text-muted-foreground text-[9px]">{d.toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", hour12: true })}</span>
      </div>
    );
  };
  ...
  const columns = [
    ...
    { key: "status",      label: "الحالة",
      render: row => (
        <span className={`px-1 py-0.5 rounded text-[9.5px] font-semibold ${row.status === "posted" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
  ...
  return (
    <div className="grid grid-cols-1 gap-2.5 lg:grid-cols-3 items-start max-w-full overflow-x-hidden p-1">
        ...
        <form onSubmit={handleSave} className="p-2.5 flex flex-col gap-2.5">
          ...
          <div className="flex flex-col gap-1.5">
            <div className="flex items-center justify-between border-b border-border/50 pb-1">
              ...
              <button type="button" onClick={() => setItems(prev => [...prev, emptyItem(defaultItemValues)])}
            ...
            <div className="grid grid-cols-2 gap-2 mt-0.5">
              <div className="flex flex-col gap-0.5">
                ...
                <div className="flex gap-1">
                  <input type="text"
                    value={paidAmount ? Number(paidAmount).toLocaleString("en-US", { numberingSystem: "latn" }) : ""}
      ...
      <div className="lg:col-span-2 flex flex-col gap-1 max-w-full overflow-x-auto no-print">
        ...
        {loading ? (
          <div className="text-center py-6 text-muted-foreground text-[11px]">جارٍ التحميل...</div>
        ) : (
          <div className="p-0.5">
            <DataTable columns={columns} data={invoices}
              ...
              actions={row => (
                <div className="flex items-center gap-0.5">
                  <button onClick={() => handlePrintDirectly(row)} title="طباعة" className="p-0.5 rounded hover:bg-accent text-primary"><Printer size={13} /></button>
                  {row.status === "draft" && (
                    <>
          ...
          <div className="invoice-book-container">
            ...
            <table className="invoice-book-table">
              ...
              <tbody>
                {viewItems.map((it, idx) => (
                  <tr key={it.id}>
                    <td>{idx + 1}</td>
                    <td className="font-bold">{fromInt(it.final_amount).toLocaleString("en-US", { numberingSystem: "latn" })}</td>
                    <td>{fromInt(it.net_weight).toLocaleString("en-US", { numberingSystem: "latn" })}</td>
                    <td>{fromInt(it.price).toLocaleString("en-US", { numberingSystem: "latn" })}</td>
                    <td>{(it.basket_count || "—").toLocaleString("en-US", { numberingSystem: "latn" })}</td>
                    <td style={{ fontWeight: '700' }}>{it.product_name}</td>
                    <td>جملة</td>
                  </tr>
                ))}
                {viewItems.length < 4 && Array.from({ length: 4 - viewItems.length }).map((_, i) => (
                  <tr key={`e-${i}`}>
                    <td>{viewItems.length + i + 1}</td><td></td><td></td><td></td><td></td><td></td><td></td>
                  </tr>
                ))}
              </tbody>
            </table>
            <div className="flex justify-between items-start mt-2 text-black text-[12px]">
              <div className="flex flex-col gap-1 text-[11px] border border-black p-2 bg-gray-50 min-w-[240px]">
                <div className="flex justify-between"><span>الحساب الإجمالي:</span><span className="font-bold">{formatMoney(viewInv.total_final)}</span></div>
                <div className="flex justify-between border-t border-dashed border-black pt-1"><span>المبلغ الواصل:</span><span className="font-bold text-green-800">{formatMoney(viewInv.paid_amount)}</span></div>
                <div className="flex justify-between border-t border-black pt-1 font-bold"><span>المتبقي :</span><span className="font-black text-red-700">{formatMoney(viewInv.remaining)}</span></div>
              </div>
